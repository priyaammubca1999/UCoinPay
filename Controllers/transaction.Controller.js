...
exports.createTransactionFunction = async (req, res) => {
    try {
        let { u_id, yc_u_id, phraseByUser, currency, fiat, website_url,secret } = req.body;
        queryHelper.findoneData("Backend", { "u_id": u_id }, {}, async (result) => {
            if (result) {
                if(ENCRYPTER(result.randomString, "DECRYPT") !== ENCRYPTER(secret,"DECRYPT")){
                    return res.status(200).send({ status: false, message: "You're not an Valid User." })
                }
